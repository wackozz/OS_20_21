...
void *producer(void *arg) {
  ...
  for (int i = 0; i < 10; i++) {
    ...
    //       pthread_self(), producer_counter, bin[producer_counter].product);
...
int main(int argc, char *argv[]) {
  ...
  if (arg == NULL || tid == NULL) {
    fprintf(stderr, "Error: unable to allocate memory with malloc().");
    exit(EXIT_FAILURE);
  }
  // initializations
  ...
  for (int i = 0; i < (N - 2) / 2; i++) {
    if ((pthread_create(tid + i, NULL, producer, (void *)arg)) != 0) {
  ...
  for (int i = (N - 2) / 2; i < N - 1; i++) {
    if ((pthread_create(tid + i, NULL, verifier, (void *)arg)) != 0) {
      perror("Verifier thread creation error:");
      exit(EXIT_FAILURE);
    }
  }
  // storageFacility creation
  if ((pthread_create(tid + (N - 1), NULL, storageFacility, (void *)arg)) !=
      0) {
    perror("storageFacility thread creation error:");
    exit(EXIT_FAILURE);
  }
  // wasteManagement creation
  if ((pthread_create(tid + N, NULL, wasteManagement, (void *)arg)) != 0) {
    perror("wasteManagement thread creation error:");
    exit(EXIT_FAILURE);
  }
